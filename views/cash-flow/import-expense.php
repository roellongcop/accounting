...
$this->title = 'Import';
$this->params['breadcrumbs'][] = ['label' => 'Expense', 'url' => Url::to(['cash-flow/expense'])];
...
<?php $form = ActiveForm::begin(['id' => 'user-form-my-account']); ?>
  <div class="row">
    <div class="col-md-6">
      <?= $form->dropzone($model, 'file', 'Expense', [
        ...
        'success' => <<< JS
          ...
          $.ajax({
            url: app.baseUrl + 'cash-flow/import-expense?process=validate',
            data: {token: token},
            method: 'post',
            dataType: 'json',
            success: function(s) {
              ...
              if (s.status === "success") {
                Swal.fire({
                  ...
                  if (result.value) {
                    ...
                    $.ajax({
                      url: app.baseUrl + 'cash-flow/import-expense?process=save',
                      data: {token: token},
                      method: 'post',
                      dataType: 'json',
                      success: function(s) {
